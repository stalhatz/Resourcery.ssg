...
SYSTEM_FONT_KEYWORDS = {
    ...
    'helvetica neue', 'arial', 'helvetica', 'sans-serif', 'serif',
    'monospace', 'cursive', 'fantasy', 'ui-sans-serif', 'ui-serif',
    'ui-monospace', 'georgia', 'times new roman', 'courier new',
    'var(--font-family)',
}
...
def read_cached_fonts(fonts_css: Path) -> list:
    """Read the font list from the metadata comment on the first line of fonts.css."""
...
def is_cache_valid(fonts_css: Path, wanted_names: list) -> bool:
    """
    Returns True if fonts.css lists exactly the wanted fonts
    and all corresponding .woff2 files are present on disk.
    No network contact required.
    """
...
def fetch_google_fonts_css(font_name: str, weights_param: str) -> str | None:
    """Fetch @font-face CSS from Google Fonts API. Returns None on failure."""
...
def find_first_downloadable(stack: str, weights_param: str) -> tuple:
    """
    Try each non-system font in the stack in order.
    Returns (winning_name, css) for the first that resolves on Google Fonts,
    or (None, None) if all candidates fail.
    Warns if the first candidate failed and a later one succeeded.
    """
...
def process_font(font_name: str, css: str, face_blocks_out: list) -> bool:
    """
    Download all woff2 variants from already-fetched CSS and append
    local @font-face rules to face_blocks_out. Returns True on success.
    """
...
def acquire_fonts():
    ...
    wanted_names = list(dict.fromkeys(
        candidates[0]
    ...
    for label, stack in [('font_family', font_family), ('heading_font', heading_font)]:
        ...
        if font_name is None:
            print(f"  ✗ No valid Google Font found in {label} stack — will rely on system fallbacks")
            all_ok = False
            continue
