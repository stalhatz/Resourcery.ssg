...
class ImageAcquirer:
    ...
    async def capture_screenshot(self, url: str, save_path: Path) -> bool:
        """Capture screenshot of webpage using Puppeteer."""
    ...
    def acquire_for_link(self, link: Dict, force: bool = False) -> Optional[str]:
        """Acquire image for a single link."""
...
def main():
    """CLI entry point for image acquisition."""
    import argparse
    
    parser = argparse.ArgumentParser(description='Acquire images for link aggregation site')
    parser.add_argument('--force', action='store_true', help='Re-acquire all images')
    parser.add_argument('--links', type=str, default='data/links.json', help='Path to links.json')
